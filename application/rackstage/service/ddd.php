...
class Evaluation extends Common
{
    ...
    public static function create_manager_tables()
    {
        ...
        $w['projectname'] = ['neq', 0];
        $w['is_quit'] = -1;
        $users = Db::name('users')->where($w)->select();
        $log = [];
        foreach ($users as $k => $v) {
            $this_project = Db::name('project')->where(['manager' => $v['id']])->find();
            // $this_project = Db::name('project')->field('type,aims')->find($v['projectname']); //当前期项�?经理项目
            ...
            switch ($project_log['status']) {
                case '1': //持效期
                    ...
                    $subscribe_rate = calc($number, $aims, 'div', 2); //认购完成�?
                    $subscribe_rate > 0.5 ? $subscribe_real = 30 : $subscribe_real = 0; //认购完成得分
                ...
                case '-1': //蓄客期
                    ...
                    $erp_project = (new ErpProject())->get_project(['project_name' => $this_project['name']]);
                    $total_oln = (new ErpCostume())->get_one_oln(['project_id' => $erp_project['id']]); //来访人数
                    $rate = calc($total_oln, $project_log['visiting'], 'div', 2);
                    $rate > 0.5 ? $develop_costume = 40 : $develop_costume = 0;
                    if ($a > 0 || $b > 0) {//
                        $authenticity = 0;
                    } else {
                        $authenticity = 40;
            ...
            $one = Db::name('manager_evaluate')->where(['project_title' => $v['projectname'], 'username' => $v['work_id'] . ', ' . $v['username'], 'month' => $month])->find();
            if (empty($one)) {
                $b = calc($subscribe_real, $authenticity, 'add', 2);
                $da = [
                    'project_title' => $v['projectname'],
                    'username' => $v['work_id'] . sysconf('username_break') . $v['username'],
    ...
    public function index(Request $request)
    {
        ...
        //        $project_name=Db::name('project')->where(['manager'=>$user_id])->value('id');//项目
        //        $project_name = Db::name('users')->where('user_id', $_SESSION['think']['user_id'])->value('projectname'); //项目
        ...
        if (isset($_GET['id'])) {
            $where['del'] = -1;
            //            $where['station'] = 19; //岗位为置业顾�?
            $where['month'] = date('Y-m', strtotime(date('Y-m-01') . " - 1 month"));
            $where['project_title']  = db::name('project')->where(['manager'=>$_SESSION['think']['user_id'],'del'=>'-1','is_edit'=>'1'])->value('id');

            $where = EmployeeService::insert_where($jurisdiction, $where, $where['project_title']); //拼�?�条�?
    ...
    public function export()
    {
        $post = input("param.");
        //设置表头：
        $arr = ['编号', '事业部', '项目', '姓名', '工号', '认购得分', '自拓客户得分', '计划书得分', 'Erp录入得分', '总分'];
        ...
        $keys = ['id', 'department', 'project_title', 'username', 'work_id', 'subscribe_real', 'oln_real_score', 'work_book_score', 'erp_entry_real_score', 'total_real'];
    ...
    public function aims(Request $request)
    {
        ...
        //        foreach($aims as $k=>$v){
        //            $bool=Db::name('project')->where(['id'=>$v['project_title'],'del'=>1])->find();
        //            if (!empty($bool)){
        //                Db::name('aims')->delete($v['id']);
        //            }
        //
        //        }
    ...
    public function ledger(Request $request)
    {
        ...
        if (isset($get['id'])) {
            ...
            $count = Db::name('clerk_upload')->where($where)->count();
            return json(['code' => '0', 'msg' => 'ok', 'data' => $list, 'limit' => $limit, 'count' => $count, 'page' => $page]);
        }
        list($pro_pid, $framework_pid) = EmployeeService::frame_project();
        $this->assign('users', Db::name('user')->field('id,user_name')->select());
